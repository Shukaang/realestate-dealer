...
export default function AdminListingsPage() {
  ...
  // Render action buttons with role checks
  const renderActionButtons = (listing: Listing) => {
    const buttons = [];
    const isCurrentlyUpdating = isUpdating === listing.id;

    // Edit button (always available but will check permission on click)
    buttons.push(
      <Tooltip key="edit">
    ...
    // Delete button (will check permission on click)
    buttons.push(
      <Tooltip key="delete">
        <TooltipTrigger asChild>
          <Button
    ...
    // Status-specific buttons (will check permission on click)
